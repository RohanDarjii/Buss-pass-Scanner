...
const Login = ({ route, navigation }) => {
    useEffect(() => {
        setformData('Login as');
    }, [])
    const [formData, setformData] = useState({
        email: '',
        password: ''
    })
    const [loginBtnText, setloginBtnText] = useState('Login as');
    const dispatch = useDispatch();
    const [error, seterror] = useState(null);
    const type = route.params?.type;

    const handleSubmit = () => {
        setloginBtnText('Login as');
        seterror(null);
        if (type === 'Student') {
            const formF = {
                ...formData,
            ...
            return fetch(baseUrl + 'api/auth', {
                ...
                .then(data => {
                    setloginBtnText('Logging in as');
                    if (data.error) {
                        setloginBtnText('Log in error');
                        seterror(data.msg);
                    }
                    else {
                        dispatch({
...

            {
                type === 'Student' ? (
                    <View style={styles.submitBtn}>
                        <Button title={`${loginBtnText} ${type}`} color='#000' onPress={handleSubmit} />
                    </View>
                ) : (
                    <View style={styles.submitBtn}>
                        <Button title={`${loginBtnText} ${type}`} color='#000' onPress={handleSubmit} />
